...
# ========================================================
# ⚠️ تحذير أمني: مفتاح API مكشوف في الكود المصدري.
#   سيتم تضمينه داخل ملف APK النهائي ويمكن استخراجه.
#   للإنتاج: استخدم Backend وسيط أو Flet secrets.
# ========================================================
...
API_TIMEOUT_SEC = 45
MAX_IMAGE_BYTES = 4 * 1024 * 1024  # 4MB لكل صورة
...
def get_storage(page, key, default=None):
    try:
        ...
        logger.warning("client_storage.get failed: %s", ex)
...
def set_storage(page, key, value):
    ...
    try:
        ...
        logger.warning("client_storage.set failed: %s", ex)


def safe_float(val, default=0.0):
    """تحويل آمن لأي قيمة إلى float مع تجاهل القيم الفارغة/النصية."""
    try:
...
def parse_gemini_json(raw_text):
    """استخراج JSON من رد الذكاء الاصطناعي بشكل قوي."""
    ...
    raise ValueError("لم يتمكن من تحويل رد الذكاء الاصطناعي إلى JSON")
...
def main(page: ft.Page):
    try:
        ...
        theme_icon = ft.IconButton(
            icon=ft.Icons.DARK_MODE, icon_color="white", on_click=toggle_theme
        )
        ...
        def calculate_iob():
            """
            حساب الأنسولين النشط (IOB) باستخدام نموذج انحلال خطي مبسّط.
            ملاحظة: النماذج الطبية الحقيقية (Walsh/Novolog) غير خطية.
            """
            history = load_history()
            now = datetime.datetime.now()
            iob = 0.0
            active_time_mins = 240  # 4 ساعات
            for record in history:
                try:
                    ...
                    logger.warning("IOB record skipped: %s", ex)
            return max(0.0, iob)
        ...
        alarm_dialog = ft.AlertDialog(
            ...
            content=ft.Text(
                "", size=18, text_align="center", weight=ft.FontWeight.BOLD
            ),
            actions=[
                ft.ElevatedButton(
                    "حسناً، تم",
        ...
        def custom_textfield(
            ...
            return ft.TextField(
                ...
                multiline=multiline,
                keyboard_type="text" if multiline else "number",
                disabled=disabled,
        ...
        ai_details_card = ft.Container(
            border_radius=20, padding=20, visible=False, content=ai_details_card_content
        )

        result_card_content = ft.Column(spacing=10)
        result_card = ft.Container(
            border_radius=20, padding=20, visible=False, content=result_card_content
        )
        ...
        upload_zone = ft.Container(
            ...
            border=ft.Border(
                top=ft.BorderSide(width=2, color="teal"),
                bottom=ft.BorderSide(width=2, color="teal"),
                left=ft.BorderSide(width=2, color="teal"),
                right=ft.BorderSide(width=2, color="teal"),
            ),
            border_radius=20,
            ink=True,
        ...
        def _build_gemini_parts():
            ...
            for path in selected_images_paths:
                ...
                if size > MAX_IMAGE_BYTES:
                    logger.warning(
                        "تخطي صورة كبيرة (%d bytes): %s", size, path
                    )
                    continue
                try:
        ...
        def _call_gemini(parts):
            """دالة متزامنة تُنفَّذ في Thread منفصل."""
        ...
        async def analyze_meal(e):
            ...
            try:
                ...
                # إرسال الطلب في thread منفصل لتجنب تجميد الواجهة
                ...
                if impact:
                    ai_details_card_content.controls.append(
                        ft.Container(
                            ...
                            padding=12,
                            border_radius=10,
                            margin=ft.Margin(left=0, right=0, top=5, bottom=0),
                        )
                    )
                ...
                page.snack_bar = ft.SnackBar(
                    ft.Text(f"خطأ من الخدمة ({ex.code})"),
                    bgcolor="red",
                )
                page.snack_bar.open = True
        ...
        def calculate_final_dose(e):
            try:
                ...
                # ⚠️ تحذير طبي إذا تجاوزت الجرعة الحد الآمن
                if capped:
                    result_card_content.controls.append(
                        ft.Container(
                            content=ft.Row(
                                [
                                    ft.Icon(
                                        ft.Icons.WARNING_AMBER, color="red"
                                    ),
                                    ft.Text(
        ...
        rem_type = ft.Dropdown(
            label="نوع التنبيه",
            options=[
                ft.dropdown.Option("med", "💊 تذكير دواء"),
                ft.dropdown.Option("appt", "📅 موعد طبي"),
                ft.dropdown.Option("refill", "🔄 إعادة صرف"),
            ],
            value="med",
            border_radius=15,
            filled=True,
        ...
        med_freq = ft.Dropdown(
            label="التكرار",
            options=[
                ft.dropdown.Option("مرة يومياً", "مرة واحدة يومياً"),
                ft.dropdown.Option("مرتين يومياً", "مرتين يومياً"),
                ft.dropdown.Option("عند الحاجة", "عند الحاجة"),
            ],
            value="مرة يومياً",
            border_radius=15,
            filled=True,
        ...
        def delete_rem(rem_id):
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text("تم الإنجاز!", weight=ft.FontWeight.BOLD), bgcolor="green"
            )
            page.snack_bar.open = True
            refresh_rems()
        ...
        # ملاحظة: هذا التنبيه يعمل فقط أثناء فتح التطبيق.
        # للإشعارات الحقيقية في الخلفية يجب استخدام إضافة نظام
        # (flutter_local_notifications) عبر Flet Extensions.
        async def alarm_background_loop():
            while True:
                try:
                    ...
                    five_mins_full = (now + datetime.timedelta(minutes=5)).strftime(
                        "%Y-%m-%d %H:%M"
                    )
                    ...
                    ft.Container(
                        content=ft.Column(
                            [
                                ...
                                ft.Text(
                                    "متوسط السكر", size=11, color="white"
                                ),
                                ft.Text(
                                    f"{round(avg_bg)}",
                                    weight=ft.FontWeight.BOLD,
