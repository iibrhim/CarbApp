...
# ========================================================
# ✅ إعدادات Gemini API (باستخدام SDK الجديد google-genai)
# ========================================================
...
def _build_ui(page: ft.Page):
    ...
    # --- قسم الحاسبة الذكية ---
    # ====================================================
    # ✅ نحتفظ بالمسار الأصلي (content://) بدلاً من base64
    selected_images = []
    ...
    def update_images_ui():
        ...
        for i, item in enumerate(selected_images):
            # ✅ استخدام src= مع content:// URI مباشرة
            img = ft.Image(
                src=item.get("path"),
                width=70,
                height=70,
        ...
        if e.files:
            ...
            for f in e.files:
                ...
                if not path:
                    ...
                    continue
                try:
                    # ✅ نحتفظ بالمسار كما هو (content:// أو file://)
        ...
        try:
            ...
            for item in selected_images:
                try:
                    ...
                    logger.warning("فشل قراءة الصورة %s: %s", item["path"], ex)

            logger.info("إرسال %d جزء للتحليل", len(parts))
            raw_text = await asyncio.to_thread(_call_gemini_sync, parts)
