...
def _build_ui(page: ft.Page):
    ...
    def update_images_ui():
        ...
        if selected_images:
            info = " | ".join(f"{it['name']}" for it in selected_images)
        ...
        try:
            ...
            for item in selected_images:
                try:
                    ...
                    logger.info("تم تحويل الصورة %s إلى base64", item["name"])
                except Exception as ex:
                    logger.warning(
                        "فشل قراءة الصورة %s: %s", item["path"], ex
                    )
            ...
            err_body = ""
            try:
                err_body = ex.read().decode("utf-8")[:400]
            except Exception:
                pass
            logger.error("Response: %s", err_body)

            if ex.code == 401 or ex.code == 403:
                msg = "❌ مفتاح Gemini غير صالح أو غير مصرح"
            elif ex.code == 404:
                msg = f"❌ الموديل غير متوفر ({GEMINI_MODEL})"
            elif ex.code == 429:
                msg = "⚠️ تجاوزت الحصة. حاول لاحقاً"
            else:
                msg = f"خطأ ({ex.code}): {err_body[:100]}"
