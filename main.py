...
def main(page: ft.Page):
    # درع الانهيار: لمنع الشاشة البيضاء وعرض أي خطأ برمجي مستقبلي على الشاشة مباشرة
    try:
        ...
        def load_history(): return get_storage(page, "user_history") or []
        def save_history(data): set_storage(page, "user_history", data)
        ...
        def update_images_ui():
            ...
            for path in selected_images_paths: 
                if path: images_row.controls.append(ft.Image(src=path, width=70, height=70, fit=ft.ImageFit.COVER, border_radius=10))
            page.update()
        ...
        def on_file_picked(e: ft.FilePickerResultEvent):
            if e.files:
                ...
                for f in e.files: 
                    if f.path: selected_images_paths.append(f.path)
                update_images_ui()
        ...
        def analyze_meal(e):
            ...
            try:
                # الاتصال المباشر النظيف والسريع
                ...
                with urllib.request.urlopen(req) as response:
                    ...
                    for item in result_data.get("items", []):
                        items_wrap.controls.append(ft.Container(content=ft.Row([ft.Icon(ft.Icons.RESTAURANT, size=12), ft.Text(f"{item['name']} ({item['weight_g']}ج)", weight=ft.FontWeight.BOLD, size=12)], spacing=4), bgcolor="bluegrey200", padding=ft.Padding(left=10, right=10, top=6, bottom=6), border_radius=15))
        ...
        def calculate_final_dose(e):
            try:
                ...
                bg = float(current_bg_input.value) if current_bg_input.value else float(get_setting("target_bg", 100.0))
                icr = float(get_setting("icr", 10.0))
                isf = float(get_setting("isf", 50.0))
                target = float(get_setting("target_bg", 100.0))
        ...
        calculator_view = ft.Container(
            ...
            content=ft.Column([
                ...
                current_bg_input, upload_zone, images_row, description_input,
                ...
                loading_ring, ai_details_card, ft.Divider(color="grey"), extracted_carbs_input,
        ...
        def handle_time1_click(e):
            if rem_type.value == "med" and med_freq.value in ["مرة يومياً", "مرتين يومياً"]: time_picker_1.pick_time()
            else: date_picker_1.pick_date()
        ...
        def on_rem_type_change(e):
            rem_time_field_1.value = ""; rem_time_field_2.value = ""
            if rem_type.value == "med":
                med_freq.visible = True; appt_day_before.visible = False
                if med_freq.value == "مرة يومياً":
                    rem_time_field_1.label = "وقت التنبيه (يومياً)"; rem_time_field_1.prefix_icon = ft.Icons.ACCESS_TIME; rem_time_container_2.visible = False; rem_time_field_2.visible = False
                elif med_freq.value == "مرتين يومياً":
                    rem_time_field_1.label = "الوقت الأول (يومياً)"; rem_time_field_1.prefix_icon = ft.Icons.ACCESS_TIME; rem_time_container_2.visible = True; rem_time_field_2.visible = True
                else:
                    rem_time_field_1.label = "الوقت والتاريخ"; rem_time_field_1.prefix_icon = ft.Icons.CALENDAR_MONTH; rem_time_container_2.visible = False; rem_time_field_2.visible = False
            else:
                med_freq.visible = False; appt_day_before.visible = True
                rem_time_field_1.label = "الوقت والتاريخ"; rem_time_field_1.prefix_icon = ft.Icons.CALENDAR_MONTH; rem_time_container_2.visible = False; rem_time_field_2.visible = False
            page.update()
        ...
        def on_time1_picked(e):
            if time_picker_1.value:
                if rem_type.value == "med" and med_freq.value in ["مرة يومياً", "مرتين يومياً"]: rem_time_field_1.value = time_picker_1.value.strftime("%H:%M")
                else: selected_dt["time"] = time_picker_1.value; rem_time_field_1.value = datetime.datetime.combine(selected_dt["date"], selected_dt["time"]).strftime("%Y-%m-%d %H:%M")
                page.update()
        ...
        def on_time2_picked(e):
            if time_picker_2.value: rem_time_field_2.value = time_picker_2.value.strftime("%H:%M"); page.update()
        ...
        def load_rems(): return get_storage(page, "user_rems") or []
        def save_rems(data): set_storage(page, "user_rems", data)
        
        def delete_rem(rem_id): 
            save_rems([d for d in load_rems() if d['id'] != rem_id])
            page.snack_bar = ft.SnackBar(ft.Text("تم الإنجاز!", weight=ft.FontWeight.BOLD), bgcolor="green"); page.snack_bar.open = True; refresh_rems()
            ...
            if not data: reminders_list.controls.append(ft.Container(content=ft.Column([ft.Icon(ft.Icons.NOTIFICATIONS_NONE, size=70, color="grey"), ft.Text("لا توجد تنبيهات", weight=ft.FontWeight.BOLD, size=18)], horizontal_alignment="center"), alignment=ft.Alignment(0.0, 0.0), padding=60))
                ...
                for item in data:
                    ...
                    card = ft.Container(
                        ...
                        ], spacing=10), bgcolor="grey100", padding=15, border_radius=15, border=ft.Border(left=ft.BorderSide(width=6, color=color))
                    )
        ...
        def delete_history_record(record_time):
            ...
            save_history(history)
            page.snack_bar = ft.SnackBar(ft.Text("تم حذف الجرعة من السجل", weight=ft.FontWeight.BOLD), bgcolor="red"); page.snack_bar.open = True; refresh_dashboard()

        def refresh_dashboard():
            dash_content.controls.clear()
            history = load_history()
            if not history:
                dash_content.controls.append(ft.Container(content=ft.Column([ft.Icon(ft.Icons.INSERT_CHART, size=60, color="grey"), ft.Text("لا توجد بيانات مسجلة", weight=ft.FontWeight.BOLD)], horizontal_alignment="center"), alignment=ft.Alignment(0.0, 0.0), padding=50)); page.update(); return
            ...
            for r in reversed(history[-15:]): 
                dash_content.controls.append(ft.Container(content=ft.ListTile(leading=ft.Icon(ft.Icons.VACCINES, color="blue"), title=ft.Text(f"الجرعة: {round(r['dose'],1)} وحدة", weight=ft.FontWeight.BOLD), subtitle=ft.Text(f"السكر: {r['bg']} | {r['time']}", size=11), trailing=ft.IconButton(icon=ft.Icons.DELETE, icon_color="red", on_click=lambda e, t=r['time']: delete_history_record(t))), bgcolor="grey100", border_radius=10))
            page.update()
        ...
        def save_settings(e):
            try:
                set_storage(page, "icr", float(icr_input.value))
                set_storage(page, "isf", float(isf_input.value))
                set_storage(page, "target_bg", float(target_bg_input.value))
        ...
        # --- 9. إدارة شريط التنقل والبناء النهائي ---
    ...
    # إذا حدث أي خطأ برمجي مستقبلاً، سيتم عرضه هنا بدلاً من الشاشة البيضاء!
        ...
        page.add(
            ft.SafeArea(
                ft.Column([
                    ft.Text("⚠️ حدث خطأ أثناء تشغيل التطبيق:", color="red", weight="bold", size=18),
                    ft.Container(
                        content=ft.Text(err_msg, color="white", size=12, selectable=True, rtl=False),
                        bgcolor="black", padding=10, border_radius=10, expand=True
                    )
